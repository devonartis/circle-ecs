...
deploy_cluster() {
    ...
    make_task_def
    register_definition
    if [[ $(aws ecs update-service --cluster wpDiscoverNC --service circle-ecs-service --task-definition $revision | \
    ...
    for attempt in {1..30}; do
        if stale=$(aws ecs describe-services --cluster wpDiscoverNC --services circle-ecs-service | \
